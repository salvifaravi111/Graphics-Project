		...
		if (vsErr) {
			...
			loadTextResource('./shader.fs.glsl', function (fsErr, fsText) {
				if (fsErr) {
					...
					loadJSONResource('./rubic1.json', function (modelErr, modelObj) {
						if (modelErr) {
							alert('Fatal error getting Susan model (see console)');
							console.error(fsErr);
						} else {
							loadImage('./box1.png', function (imgErr, img) {
								if (imgErr) {
...
var RunDemo = function (vertexShaderText, fragmentShaderText, SusanImage, SusanModel) {
	...
	mat4.lookAt(viewMatrix, [8, 3, -8], [0, 0, 0], [0, 1, 0]);
	...
	var specularIntensityUniformLocation = gl.getUniformLocation(program, 'specularIntensity');

	gl.uniform3f(amiibientLightIntensityUniformLocation, 0.7,0.7,0.7);
	gl.uniform3f(sunlightDirUniformLocation, 3.0,4.0,-2.0);
	gl.uniform3f(sunLightIntensityUniformLocation, 0.6,0.6,0.6);
	gl.uniform3f(specularIntensityUniformLocation, 0.9, 0.9, 0.9);
	//
	// Main render loop
	//
	...
	/*if(first_time==1){
		...
		mat4.rotate(yRotationMatrix, identityMatrix, -3.14159, [0, 1, 0]);
		mat4.rotate(xRotationMatrix, identityMatrix, -1.5708, [1, 0, 0]);

		mat4.scale(scaleMatrix, identityMatrix, [1,1,1]);

		mat4.translate(transMatrix, identityMatrix, [0,1,0]);
		...
		if (vsErr) {
			...
			loadTextResource('./shader.fs.glsl', function (fsErr, fsText) {
				if (fsErr) {
					...
					loadJSONResource('./rubic2.json', function (modelErr, modelObj) {
						if (modelErr) {
							alert('Fatal error getting Susan model (see console)');
							console.error(fsErr);
						} else {
							loadImage('./box2.png', function (imgErr, img) {
								if (imgErr) {
									alert('Fatal error getting Susan texture (see console)');
									console.error(imgErr);
								} else { 
		...
		if (vsErr) {
			...
			loadTextResource('./shader.fs.glsl', function (fsErr, fsText) {
				if (fsErr) {
					...
					loadJSONResource('./rubic3.json', function (modelErr, modelObj) {
						if (modelErr) {
							alert('Fatal error getting Susan model (see console)');
							console.error(fsErr);
						} else {
							loadImage('./box3.png', function (imgErr, img) {
								if (imgErr) {
									alert('Fatal error getting Susan texture (see console)');
									console.error(imgErr);
								} else { 
		...
		if (vsErr) {
			...
			loadTextResource('./shader.fs.glsl', function (fsErr, fsText) {
				if (fsErr) {
					...
					loadJSONResource('./rubic4.json', function (modelErr, modelObj) {
						if (modelErr) {
							alert('Fatal error getting Susan model (see console)');
							console.error(fsErr);
						} else {
							loadImage('./box4.png', function (imgErr, img) {
								if (imgErr) {
									alert('Fatal error getting Susan texture (see console)');
									console.error(imgErr);
								} else { 
		...
		if (vsErr) {
			...
			loadTextResource('./shader.fs.glsl', function (fsErr, fsText) {
				if (fsErr) {
					...
					loadJSONResource('./rubic5.json', function (modelErr, modelObj) {
						if (modelErr) {
							alert('Fatal error getting Susan model (see console)');
							console.error(fsErr);
						} else {
							loadImage('./box5.png', function (imgErr, img) {
								if (imgErr) {
									alert('Fatal error getting Susan texture (see console)');
									console.error(imgErr);
								} else { 
		...
		if (vsErr) {
			...
			loadTextResource('./shader.fs.glsl', function (fsErr, fsText) {
				if (fsErr) {
					...
					loadJSONResource('./rubic6.json', function (modelErr, modelObj) {
						if (modelErr) {
							alert('Fatal error getting Susan model (see console)');
							console.error(fsErr);
						} else {
							loadImage('./box6.png', function (imgErr, img) {
								if (imgErr) {
									alert('Fatal error getting Susan texture (see console)');
									console.error(imgErr);
								} else { 
