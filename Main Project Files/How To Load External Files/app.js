		...
		if (vsErr) {
			...
			loadTextResource('./shader.fs.glsl', function (fsErr, fsText) {
				if (fsErr) {
					...
					loadJSONResource('./test.json', function (modelErr, modelObj) {
						if (modelErr) {
							alert('Fatal error getting Susan model (see console)');
							console.error(fsErr);
						} else {
							loadImage('./bake.png', function (imgErr, img) {
								if (imgErr) {
...
var RunDemo = function (vertexShaderText, fragmentShaderText, SusanImage, SusanModel) {
	...
	mat4.lookAt(viewMatrix, [0, 3, lookAt_z], [0, 0, 0], [0, 1, 0]);
	...
	gl.uniform3f(amiibientLightIntensityUniformLocation, 0.2,0.2,0.2);
	...
	mat4.scale(scaleMatrix, identityMatrix, [5,5,5]);

	mat4.translate(transMatrix, identityMatrix, [-0.5,0,0]);
	...
	function keysPressed(e) {
	    ...
	    if (keys[37]) {
	      deltaX -= 0.5;
	      mat4.translate(transMatrix, identityMatrix, [deltaX,0,0]);
	    }
	    ...
	    if (keys[38]) {
	      deltaY -= 0.5;
	      mat4.translate(transMatrix, identityMatrix, [0,deltaY,0]);
	    }
	...
	if(flag > 10){
		window.location.pathname = "D:/WORK/Gfx project/How To Load External Files4/index.html";
	}
